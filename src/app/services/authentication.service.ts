...
import {UserInSession} from '../models/user-in-session';
...
export class AuthenticationService {
    ...
    constructor(private keycloak: Keycloak, private http: HttpClient, configurationService: ConfigurationService) {
        configurationService.getValue(LOGIN_API_PATH).pipe(take(1)).subscribe(
            path => this.logi_api_path = path
        );
        configurationService.getValue(CHANGEPASSWORD_API_PATH).pipe(take(1)).subscribe(
            change_path => this.changepassword_api_path = change_path
        );
        configurationService.getValue(RESETPASSWORD_API_PATH).pipe(take(1)).subscribe(
